...
*/
import { useRef, forwardRef, RefObject, useLayoutEffect, useState, useCallback, useEffect, ForwardedRef, CSSProperties, useMemo } from 'react';
...
interface CardProps {
  ...
  onResize: (cardId: string, dimensions: { width: number; height: number; }) => void;
  enableHoverEffect: boolean;
  globalScale: number;
}

const Card = forwardRef<HTMLDivElement, CardProps>(({ card, isDragging, isFocused, isDimmed, eventHandlers, onResize, enableHoverEffect, globalScale }, ref) => {
...
}, [onResize, card.content, card.id]);


  useLayoutEffect(() => {
    if (!elRef.current) return;
    gsap.to(elRef.current, {
      scale: isDimmed ? card.position.scale * globalScale * 0.95 : card.position.scale * globalScale,
  ...
  const handlePointerEnter = (e: React.PointerEvent<HTMLDivElement>) => {
    if (isDragging || isFocused || isDimmed || !enableHoverEffect) return;
...
const ParallaxLayer = forwardRef<HTMLDivElement, ParallaxLayerProps>(
  ({ className, cards, config, cardRefs, getCardEventHandlers, focusedCardId, onCardResize, TILE_OFFSETS, globalScale }, ref) => {
    return (
      <div className={`parallax-layer ${className}`} ref={ref}>
        {TILE_OFFSETS.map((offset, i) => (
          ...
          >
            {cards.map(card => {
              const { isDragging, eventHandlers, enableHoverEffect } = getCardEventHandlers(card, i);
...
function useParallax(
  ...
  const { globalScale, enableDragPan, enableScrollPan, enableCardDrag, enableFocusClick, enableZoom, enableHoverEffect } = interactionConfig;
  ...
  const getCardEventHandlers = useCallback((card: CardData, tileIndex: number) => ({
      ...
      eventHandlers: {
        onPointerDown: enableCardDrag 
            ? (e: React.PointerEvent<HTMLDivElement>) => handlePointerDown(card, tileIndex, e)
            : () => {},
      },
      enableHoverEffect,
  ...
  useLayoutEffect(() => {
    ...
    const updateLayout = () => {
      ...
      layerRefs.current?.forEach(layer => {
        ...
        tiles.forEach((tile, i) => {
          ...
          gsap.set(tile, { transform: `translate(${offset.x * (newWorldWidth + tileGap)}px, ${offset.y * (newWorldHeight + tileGap)}px)` });
        });
      });
    ...
    const engine = createInteractionEngine(container, {
      onDrag: ({ deltaX, deltaY }) => {
        if (!enableDragPan || interactionState.current.mode !== 'IDLE') return;
    ...
    const ticker = gsap.ticker.add(() => {
      ...
      if (currentFocusedId && currentFocusedTileIndex !== null) {
        ...
        if (card) {
            ...
            const cardWorldCenterX = cardCenterInTileX + (tileOffset.x * (worldSize.current.width + tileGap));
            const cardWorldCenterY = cardCenterInTileY + (tileOffset.y * (worldSize.current.height + tileGap));
            ...
            const cardActualWidth = cardUnscaledWidth * card.position.scale * globalScale;
            const cardActualHeight = cardUnscaledHeight * card.position.scale * globalScale;
            ...
            const panPeriodX = layerConfig.speed > 0 ? (worldSize.current.width + tileGap) / layerConfig.speed : 0;
            const panPeriodY = layerConfig.speed > 0 ? (worldSize.current.height + tileGap) / layerConfig.speed : 0;
      ...
      layerElements.forEach((layer, i) => {
        ...
        gsap.set(layer, { x: wrap(targetX, worldSize.current.width + tileGap), y: wrap(targetY, worldSize.current.height + tileGap), z: layerConfig.baseZ });
      });
        ...
        TILE_OFFSETS.forEach((_, tileIndex) => {
          ...
          if (cardElement) {
            gsap.to(cardElement, {
              x: liveX,
              y: liveY,
              scale: cardConfig.position.scale * globalScale * 1.05,
              z: 100,
          ...
          const cardConfig = cards.find(c => c.id === lastDraggedCardId.current);
          if (cardConfig) {
              TILE_OFFSETS.forEach((_, tileIndex) => {
  ...
  }, [cards, config, cardDimensions, interactionConfig, tilingConfig, TILE_OFFSETS]); // Rerun effect if cards, config or dimensions change
...
const defaultProps = {
    ...
    cards: [
        { id: 'c1', layer: 2, title: 'Card 1', meta: 'Foreground', position: { x: 150, y: -50, scale: 1.3 } },
        { id: 'c2', layer: 1, title: 'Card 2', meta: 'Middleground', position: { x: 400, y: 450, scale: 1.1 } },
        { id: 'c3', layer: 0, title: 'Card 3', meta: 'Background', position: { x: 700, y: -500, scale: 1.2 } },
    ],
    contentSlots: [],
}
...
export function InfiniteCanvas({
  ...
  const handleCardResize = useCallback((cardId: string, dimensions: { width: number, height: number }) => {
    setCardDimensions(prev => {
        if (prev[cardId]?.width === dimensions.width && prev[cardId]?.height === dimensions.height) {
            return prev;
        }
    ...
    return (rawCards || []).map((card, index) => {
      ...
      if (!stableId) {
        if (!idMapRef.current.has(card)) {
          idMapRef.current.set(card, `c${Math.random().toString(36).substring(2, 9)}`);
        }
  ...
  const TILE_OFFSETS = useMemo(() => generateTileOffsets(tiling.gridSize), [tiling.gridSize]);

  const config = useMemo(() => ({
    layers: [
      { speed: layout.layerFar.speed, baseZ: layout.layerFar.baseZ, className: 'layer-far' },
      { speed: layout.layerMid.speed, baseZ: layout.layerMid.baseZ, className: 'layer-mid' },
      { speed: layout.layerNear.speed, baseZ: layout.layerNear.baseZ, className: 'layer-near' },
    ],
  ...
  const { getCardEventHandlers, setFocusedCard, focusedCardId } = useParallax(containerRef, layerRefs, sceneRef, cardRefs, cards, config, cardDimensions, interaction, tiling, TILE_OFFSETS);
...
addPropertyControls(InfiniteCanvas, {
        ...
        control: {
            ...
            controls: {
                ...
                position: {
                    ...
                    title: "Position",
                    controls: {
                        x: { type: ControlType.Number, title: "X", defaultValue: 0 },
                        y: { type: ControlType.Number, title: "Y", defaultValue: 0 },
                        scale: { type: ControlType.Number, title: "Scale", defaultValue: 1, min: 0.1, max: 5, step: 0.1 },
                    }
                },
            }
        }
    ...
    appearance: {
        ...
        title: "Appearance",
        controls: {
            backgroundColor: { type: ControlType.Color, title: "Background", defaultValue: "#f8f8f8" },
            cardColor: { type: ControlType.Color, title: "Card Body", defaultValue: "#ffffff" },
            cardBorderColor: { type: ControlType.Color, title: "Card Border", defaultValue: "#f0f0f0" },
            cardHeaderColor: { type: ControlType.Color, title: "Card Header", defaultValue: "rgba(255, 255, 255, 0.95)" },
            cardTitleColor: { type: ControlType.Color, title: "Title Text", defaultValue: "#333333" },
            cardMetaColor: { type: ControlType.Color, title: "Meta Text", defaultValue: "#999999" },
        }
    },
    interaction: {
        type: ControlType.Object,
        title: "Interaction",
        controls: {
            globalScale: { type: ControlType.Number, title: "Global Scale", defaultValue: 1, min: 0.1, max: 3, step: 0.05 },
            scrollSpeed: { type: ControlType.Number, title: "Scroll Speed", defaultValue: 1, min: 0.1, max: 5, step: 0.1 },
            enableDragPan: { type: ControlType.Boolean, title: "Drag Pan", defaultValue: true },
            enableScrollPan: { type: ControlType.Boolean, title: "Scroll Pan", defaultValue: true },
            enableCardDrag: { type: ControlType.Boolean, title: "Drag Cards", defaultValue: true },
            enableFocusClick: { type: ControlType.Boolean, title: "Click to Focus", defaultValue: true },
            enableZoom: { type: ControlType.Boolean, title: "Zoom", defaultValue: true },
            enableHoverEffect: { type: ControlType.Boolean, title: "Hover Effect", defaultValue: true },
        }
    },
    layout: {
        type: ControlType.Object,
        ...
        controls: {
            layerFar: {
                ...
                title: "Far Layer (0)",
                controls: {
                    speed: { type: ControlType.Number, title: "Speed", defaultValue: 0.3, min: 0, max: 2, step: 0.1 },
                    baseZ: { type: ControlType.Number, title: "Z-Depth", defaultValue: -600, min: -2000, max: 0, step: 10 },
                }
            },
            layerMid: {
                type: ControlType.Object,
                title: "Mid Layer (1)",
                controls: {
                    speed: { type: ControlType.Number, title: "Speed", defaultValue: 0.6, min: 0, max: 2, step: 0.1 },
                    baseZ: { type: ControlType.Number, title: "Z-Depth", defaultValue: -300, min: -2000, max: 0, step: 10 },
                }
            },
            layerNear: {
                type: ControlType.Object,
                title: "Near Layer (2)",
                controls: {
                    speed: { type: ControlType.Number, title: "Speed", defaultValue: 1.0, min: 0, max: 2, step: 0.1 },
                    baseZ: { type: ControlType.Number, title: "Z-Depth", defaultValue: 0, min: -2000, max: 0, step: 10 },
                }
            },
        }
    },
    tiling: {
        ...
        controls: {
            gridSize: {
                type: ControlType.Number,
                title: "Grid Size",
                description: "The number of tiles in each direction (odd numbers work best).",
                defaultValue: 5,
                min: 1,
                max: 9,
                step: 2,
