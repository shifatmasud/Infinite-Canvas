...
function useParallax(
    ...
    useLayoutEffect(() => {
        ...
        const updateLayout = () => {
            ...
            layerRefs.current?.forEach((layer) => {
                ...
                tiles.forEach((tile, i) => {
                    ...
                    gsap.set(tile, {
                        transform: `translate(${offset.x * newWorldWidth}px, ${offset.y * newWorldHeight}px)`,
                    })
                })
            })
        ...
        const ticker = gsap.ticker.add(() => {
            ...
            if (currentFocusedId && currentFocusedTileIndex !== null) {
                ...
                if (card) {
                    ...
                    const cardWorldCenterX =
                        cardCenterInTileX +
                        tileOffset.x * worldSize.current.width
                    const cardWorldCenterY =
                        cardCenterInTileY +
                        tileOffset.y * worldSize.current.height
                    ...
                    const panPeriodX =
                        layerConfig.speed > 0
                            ? worldSize.current.width / layerConfig.speed
                            : 0
                    const panPeriodY =
                        layerConfig.speed > 0
                            ? worldSize.current.height / layerConfig.speed
                            : 0
            ...
            layerElements.forEach((layer, i) => {
                ...
                gsap.set(layer, {
                    x: wrap(targetX, worldSize.current.width),
                    y: wrap(targetY, worldSize.current.height),
                    z: layerConfig.baseZ,
                })
            })
    ...
    }, [cards, config, cardDimensions, interactionConfig]) // Rerun effect if cards, config or dimensions change
